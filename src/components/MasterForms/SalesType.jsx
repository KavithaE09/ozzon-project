...
export default function SalesType() {
  ...
  const [salesTypeName, setSalesTypeName] = useState('');
  ...
  const handleSubmit = () => {
    if (salesTypeName.trim() !== '') {
      if (editingId) {
        // Update existing record
        const updatedTypes = salesTypes.map(type =>
          type.id === editingId ? { ...type, name: salesTypeName.toUpperCase() } : type
        );
        setSalesTypes(updatedTypes);
        setFilteredGroups(updatedTypes);
        setEditingId(null);
      } else {
        // Add new record
        const newType = {
          id: salesTypes.length > 0 ? Math.max(...salesTypes.map(g => g.id)) + 1 : 1,
          name: salesTypeName.toUpperCase()
        };
        setSalesTypes([...salesTypes, newType]);
        setFilteredGroups([...salesTypes, newType]);
      }
      setSalesTypeName('');
      setCurrentPage(1);
    }
  ...
  const handleEdit = (type) => {
    ...
    setSalesTypeName(type.name);
    window.scrollTo({ top: 0, behavior: 'smooth' });
  };

  const handleDelete = (typeId) => {
    const updatedTypes = salesTypes.filter(type => type.id !== typeId);
    setSalesTypes(updatedTypes);
    setFilteredGroups(updatedTypes);
    setCurrentPage(1);
  };
      ...
      <div className="content-wrapper">
        <div className="main-section">
          <div className="content-card">
            {/* Sales Type Section */}
            <h2 className="page-title">Sales Type</h2>
            
            <div className="filter-section">
              <div className="filter-grid">
                <div className="filter-grid-red">
                  <label className="filter-label">Sales Type List</label>
                  <input
                    type="text"
                    value={salesTypeName}
                    onChange={(e) => setSalesTypeName(e.target.value)}
            ...
            <div className="master-table-container">
              <div className="master-table-header">
                <span className="master-table-title">Sales Type List</span>
              </div>
